...
export default function Home() {
  ...
  useEffect(() => {
    // Fallback: Make sections visible after a short delay if IntersectionObserver fails
    ...
    if (typeof IntersectionObserver === "undefined") {
      // Browser doesn't support IntersectionObserver, show all sections immediately
      sectionsRef.current.forEach((section) => {
        if (section) {
          section.classList.add("animate-fade-in-up")
        }
  ...
  /**
   * Parses markdown-style bold syntax (**text**) and returns JSX with styled spans.
   * Can be used for any text field in the portfolio (intro, projects, work, etc.)
   * Text wrapped in ** will be rendered with text-foreground color for emphasis.
   */
    ...
    <div className="min-h-screen bg-background text-foreground relative">
      ...
      <nav className="fixed left-8 top-1/2 -translate-y-1/2 z-10 hidden lg:block">
        <div className="flex flex-col gap-4">
          {["intro", "work", "projects", ...(data.thoughts.length > 0 ? ["thoughts"] : []), "connect"].map((section) => (
            <button
              key={section}
      ...
      <main className="max-w-4xl mx-auto px-6 sm:px-8 lg:px-16">
          ...
          >
            <div className="space-y-12 sm:space-y-16">
              ...
              <div className="grid gap-6 sm:gap-8 lg:grid-cols-2">
                {(data.thoughts as ThoughtPost[]).map((post, index) => (
                  <article
                    key={index}
                  ...
                  >
                    <div className="space-y-4">
                      ...
                      <p className="text-muted-foreground leading-relaxed">{parseMarkdownBold(post.excerpt)}</p>
        ...
        <section
          id="connect"
          ref={(el) => {
            sectionsRef.current[data.thoughts.length > 0 ? 4 : 3] = el
          }}
